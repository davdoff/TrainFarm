...
from src.detectors.color_detector import ColorDetector
import pytesseract
...
class ResourceGenerator:
    ...
    def __init__(self):
        self.trains_bottom_template = "Templates/tasks/allTrainInBottomOfScreen.png"
        self.dispatch_button_template = "Templates/buttons/DispatchButton.png"
        self.color_detector = ColorDetector()
        self.confirm_button_template = "Templates/buttons/ConfirmButton.png"
    ...
    def collect_from_mine(self) -> bool:
        ...
        # Verify train is clicked (check for Dispatch button in bottom-left region)
        if self.verify_train_clicked():
            print("✅ Train selected successfully (Dispatch button visible)")
        else:
            print("⚠️  Train might not be selected (no Dispatch button), continuing anyway...")
    ...
    def verify_train_clicked(self) -> bool:
        """
        Verify that a train is selected by looking for the Dispatch button.
        The Dispatch button appears in the bottom-left area when a train is selected.

        Returns:
            True if train is selected (dispatch button found), False otherwise
        """
        # Define bottom-left search region (20% from left, 80% down)
        screen_width, screen_height = pyautogui.size()
        region_x = 0
        region_y = int(screen_height * 0.8)  # Start at 80% down
        region_width = int(screen_width * 0.2)  # Search 20% from left
        region_height = screen_height - region_y  # To bottom of screen

        match = find_template_on_screen(
            self.dispatch_button_template,
            threshold=0.6,
    ...
    def find_dispatch_button(self) -> Optional[Tuple[int, int]]:
        """
        Find the Dispatch button in the bottom-left region of the screen.
        Searches in a designated area (20% from left, 80% down) for efficiency.
        ...
        # Fallback: Estimate position in bottom-left area
        screen_width, screen_height = pyautogui.size()
        estimated_x = int(screen_width * 0.1)  # 10% from left (middle of 20% region)
        estimated_y = int(screen_height * 0.9)  # 90% down (middle of bottom 20%)
    ...
    def find_blue_button(self) -> Optional[Tuple[int, int]]:
        ...
        Searches in center region only (40-70% horizontal, 40-80% vertical)
        where popups appear for better performance.
        ...
        # Define search region (center area where popups appear)
        screen_width, screen_height = pyautogui.size()
        region_x = int(screen_width * 0.4)      # 40% from left
        region_y = int(screen_height * 0.4)     # 40% from top
        region_width = int(screen_width * 0.3)  # 30% width (40% to 70%)
        region_height = int(screen_height * 0.4) # 40% height (40% to 80%)

        print(f"Search region: X={region_x}-{region_x + region_width}, Y={region_y}-{region_y + region_height}")
        print(f"  (40-70% horizontal, 40-80% vertical)")
        ...
        if contours:
            ...
            # Collect all valid blue button candidates
            valid_buttons = []
            for contour in contours[:3]:
                ...
                # Calculate center position (accounting for scale factor AND region offset)
                center_x = int((x + w // 2) / scale_factor) + region_x
                center_y = int((y + h // 2) / scale_factor) + region_y
            ...
            if valid_buttons:
                # Sort by Y position (ascending) - HIGHEST button has smallest Y
                valid_buttons.sort(key=lambda b: b['y'])
                highest_button = valid_buttons[0]

                print(f"Found {len(valid_buttons)} valid blue button(s)")
                print(f"✅ Clicking HIGHEST button: size={highest_button['width']}x{highest_button['height']}, "
                      f"area={highest_button['area']}, at ({highest_button['x']}, {highest_button['y']})")
                return (highest_button['x'], highest_button['y'])
